...
async function getPracticePapers() {
  try {
    ...
    const res = await fetch(`${baseUrl}/api/materials/practice-papers`, {
      cache: "no-store",
    });
    if (!res.ok) {
      return { materials: [] };
...
export default async function PracticePapersPage() {
  const data = await getPracticePapers();
      ...
      <div className="bg-gradient-to-br from-neutral-50 via-neutral-0 to-accent-yellow-light">
        <div className="container mx-auto px-6 sm:px-8 lg:px-12 py-8 sm:py-12">
          ...
          <p className="text-lg text-neutral-600 mb-8 max-w-3xl">
            Full-length practice tests to help you prepare for the actual IELTS
            examination.
          </p>
